...
%  alpha0 = alpha0/norm(alpha0);
  beta_ = n(i, : ) - dot(n(i, : ), alpha0)*alpha0/norm(alpha0);
...
%           norm(cross(cross(x(1:3)', x(4:6)'), kn(i, : ) / norm(kn(i, : ))));
  f = @(x) norm(x(1:3)'-k(i, : ) / norm(k(i, : )))^2 + ...
           ...
           norm(cross(x(1:3)', x(4:6)')-kn(i, :)) / norm(kn(i, :))^2;
...
%  plot3( [0 kn(i, 1)], [0 kn(i, 2)], [0 kn(i, 3)], 'b', 'marker', '^');
  plot3( [0 cross(k(i, :), n(i, :))(1)], [0 cross(k(i, :), n(i, :))(2)], [0 cross(k(i, :), n(i, :))(3)], 'b', 'marker', '^');
